...
(() => {
  ...
  (() => {
    ...
    var UI_IDS = Object.freeze({
      ...
      primary: "Moeen-2-primary-btn",
      aiBtn: "Moeen-2-ai-btn",
      quickBtn: "Moeen-2-quick-btn",
      advanced: "Moeen-2-advanced-btn",
    ...
    function injectControlPanel() {
      ...
      if (!document.getElementById("Moeen-2-font")) {
        const link = document.createElement("link");
        link.id = "Moeen-2-font";
        link.rel = "stylesheet";
      ...
      primary.innerHTML = `<span class="Moeen-2-btn-icon">${playIconSVG()}</span>
     <span class="Moeen-2-btn-label">\u062A\u062D\u0636\u064A\u0631 \u0627\u0644\u062F\u0631\u0633 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B</span>`;
      const aiBtn = document.createElement("button");
      aiBtn.id = UI_IDS.aiBtn;
      aiBtn.type = "button";
      applyButtonStyle(aiBtn, "linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)");
      aiBtn.innerHTML = `<span class="Moeen-2-btn-icon">${aiIconSVG()}</span>
     <span class="Moeen-2-btn-label">\u{1F916} \u062A\u062D\u0636\u064A\u0631 \u0627\u0644\u062F\u0631\u0633 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B</span>`;
      const quickBtn = document.createElement("button");
      quickBtn.id = UI_IDS.quickBtn;
      quickBtn.type = "button";
      applyButtonStyle(quickBtn, "linear-gradient(135deg, #1a9448 0%, #0f7a36 100%)");
      quickBtn.innerHTML = `<span class="Moeen-2-btn-icon">\u26A1</span>
     <span class="Moeen-2-btn-label">\u062A\u062D\u0636\u064A\u0631 \u0633\u0631\u064A\u0639</span>`;
    ...
    <div style="display:flex;gap:6px;margin-bottom:4px">
      <div class="Moeen-2-step" style="flex:1;height:4px;border-radius:2px;background:rgba(0,0,0,0.12);transition:background .3s"></div>
      <div class="Moeen-2-step" style="flex:1;height:4px;border-radius:2px;background:rgba(0,0,0,0.12);transition:background .3s"></div>
    </div>
    <div style="display:flex;gap:6px">
      <div class="Moeen-2-step-label" style="flex:1;font-size:10px;color:#aaa;text-align:center;transition:color .3s">\u0627\u0644\u062E\u0637\u0648\u0629 \u0661</div>
      <div class="Moeen-2-step-label" style="flex:1;font-size:10px;color:#aaa;text-align:center;transition:color .3s">\u0627\u0644\u062E\u0637\u0648\u0629 \u0662</div>
    </div>`;
      ...
      status.innerHTML = `<span class="Moeen-2-status-dot" style="width:7px;height:7px;border-radius:50%;flex-shrink:0;margin-top:5px;background:#8a8a8a;transition:background .25s"></span>
     <span class="Moeen-2-status-text">\u062C\u0627\u0647\u0632 \u0644\u0625\u0639\u062F\u0627\u062F \u0627\u0644\u062F\u0631\u0633.</span>`;
      ...
      // ── Resource-selection checkbox row ──────────────────────────────────────
      const resourceSection = document.createElement("div");
      resourceSection.id = "Moeen-2-resource-toggles";
      ...
      resLabel.style.cssText = "font-size:10px;color:#6b7c93;margin-bottom:6px;font-weight:600;text-align:right";
      resLabel.textContent = "\u0627\u0644\u0645\u0648\u0627\u0631\u062F \u0627\u0644\u062A\u064A \u0633\u064A\u062A\u0645 \u0625\u0636\u0627\u0641\u062A\u0647\u0627:"; // الموارد التي سيتم إضافتها:
      const resRow = document.createElement("div");
      resRow.style.cssText = "display:flex;flex-wrap:wrap;gap:5px;justify-content:flex-end";
      const _resDefs = [
        { key: 'activity', label: '\u0646\u0634\u0627\u0637', color: '#1a6fd4' }, // نشاط
        { key: 'homework', label: '\u0648\u0627\u062C\u0628', color: '#1a9448' }, // واجب
        { key: 'exam', label: '\u0627\u062E\u062A\u0628\u0627\u0631', color: '#c87f0a' }, // اختبار
        { key: 'enrichment', label: '\u0625\u062B\u0631\u0627\u0621', color: '#7c3aed' }  // إثراء
      ];
      function _applyChipStyle(chip, cb, color) {
        chip.style.background = cb.checked ? 'rgba(0,0,0,0.05)' : '#fafafa';
        chip.style.borderColor = cb.checked ? color : 'rgba(0,0,0,0.1)';
        chip.style.color = cb.checked ? '#16324f' : '#9ca3af';
        chip.style.opacity = cb.checked ? '1' : '0.65';
      }
      for (var _rd of _resDefs) {
        (function (rd) {
          ...
          dot.style.cssText = "width:6px;height:6px;border-radius:50%;background:" + rd.color + ";flex-shrink:0";
          var cb = document.createElement("input");
          cb.type = "checkbox";
          cb.id = "Moeen-2-res-" + rd.key;
          cb.style.cssText = "width:13px;height:13px;cursor:pointer;accent-color:" + rd.color;
          cb.checked = getResourceEnabled(rd.key);
          _applyChipStyle(chip, cb, rd.color);
          cb.addEventListener("change", function () {
            setResourceEnabled(rd.key, cb.checked);
            _applyChipStyle(chip, cb, rd.color);
          });
          var txt = document.createElement("span");
          txt.textContent = rd.label;
          chip.append(cb, dot, txt);
          resRow.appendChild(chip);
        })(_rd);
      }
      resourceSection.append(resLabel, resRow);
      // ─────────────────────────────────────────────────────────────────────────

      container.append(header, divider, primary, aiBtn, quickBtn, quickTooltip, advanced, resourceSection, stepBar, status);
    ...
    function updateDashboardCounter() {
      ...
      if (saveBtn) {
        var _anyRes = Object.keys(_Moeen2ResEnabled).some(function (k) { return _Moeen2ResEnabled[k]; });
        var _canSave = selected > 0 && _anyRes;
        saveBtn.disabled = !_canSave;
        saveBtn.style.opacity = !_canSave ? "0.45" : "1";
        saveBtn.style.cursor = !_canSave ? "not-allowed" : "pointer";
        saveBtn.style.boxShadow = !_canSave
          ? "0 4px 14px rgba(26,111,212,0.2)"
          : "0 6px 20px rgba(26,111,212,0.45)";
        if (!_canSave) {
          saveBtn.classList.remove("Moeen-2-save-pulse");
        } else {
          saveBtn.classList.add("Moeen-2-save-pulse");
        }
      }
      // Show hint only when all resource pills are off
      var _hint = document.getElementById("Moeen-2-res-hint");
      if (_hint) {
        var _allOff = !Object.keys(_Moeen2ResEnabled).some(function (k) { return _Moeen2ResEnabled[k]; });
        _hint.style.display = _allOff ? "inline-flex" : "none";
      }
    }
    ...
    function injectDashboardPanel() {
      ...
      if (!document.getElementById("Moeen-2-dashboard-styles")) {
        ...
        style.textContent = [
          "@keyframes Moeen-2SlideUp { from { transform:translateY(110%);opacity:0 } to { transform:translateY(0);opacity:1 } }",
          "@keyframes Moeen-2DotPulse { 0%,100% { opacity:1;transform:scale(1) } 50% { opacity:0.35;transform:scale(0.65) } }",
          "@keyframes Moeen-2SavePulse { 0%,100% { box-shadow:0 6px 20px rgba(26,111,212,0.45) } 50% { box-shadow:0 6px 28px rgba(26,111,212,0.75),0 0 0 6px rgba(26,111,212,0.12) } }",
          ".Moeen-2-dashboard-select:hover { border-color:#1a6fd4 !important; }",
          ".Moeen-2-dashboard-badge { display:inline-flex;align-items:center;gap:6px;background:rgba(26,111,212,0.09);color:#1a6fd4;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:700;border:1px solid rgba(26,111,212,0.2);flex-shrink:0; }",
          ".Moeen-2-save-pulse { animation:Moeen-2SavePulse 2.2s ease-in-out infinite; }",
          "#Moeen-2-dashboard-save:not(:disabled):hover { transform:translateY(-2px) scale(1.02) !important; }",
          ".Moeen-2-res-pill { display:inline-flex;align-items:center;gap:5px;cursor:pointer;border-radius:20px;padding:5px 12px;font-size:12px;font-weight:700;font-family:Cairo,Segoe UI,Tahoma,sans-serif;transition:all .2s cubic-bezier(.16,1,.3,1);user-select:none;white-space:nowrap;border:1.5px solid transparent;position:relative; }",
          ".Moeen-2-res-pill:active { transform:scale(0.95); }",
          ".Moeen-2-res-pill input[type=checkbox] { position:absolute;opacity:0;width:0;height:0;pointer-events:none; }",
          ".Moeen-2-res-pill-check { font-size:10px;font-weight:900;transition:all .15s;line-height:1; }"
        ].join("\n");
      ...
      panel.style.cssText = [
        ...
        "box-shadow:0 -2px 0 rgba(26,111,212,0.08),0 -8px 40px rgba(15,23,42,0.13)",
        "font-family:Cairo,Segoe UI,Tahoma,sans-serif",
        "direction:rtl",
        "animation:Moeen-2SlideUp 0.45s cubic-bezier(0.16,1,0.3,1)"
      ].join(";");

      // Rainbow top accent bar
      var topAccent = document.createElement("div");
      topAccent.style.cssText = "height:3px;background:linear-gradient(90deg,#7c3aed 0%,#1a6fd4 35%,#1a9448 65%,#c87f0a 100%);width:100%";

      // 7-day future limit warning (Madrasati rule)
      var sevenDayWarning = document.createElement("div");
      sevenDayWarning.style.cssText = "background:rgba(200,127,10,0.08);border:1px solid rgba(200,127,10,0.25);border-radius:6px;padding:6px 12px;font-size:12px;color:#7a4d05;display:flex;align-items:center;gap:8px";
      sevenDayWarning.innerHTML = '<span style="font-size:14px">⚠️</span><span><strong>تنبيه هام:</strong> يمكنك إعداد الحصص لـ <strong>7 أيام مستقبلية فقط</strong>. الحصص التي تتجاوز هذا الحد لن يتم حفظها.</span>';
      ...
      // Student Report feature banner (new feature)
      var studentReportBanner = document.createElement("div");
      studentReportBanner.style.cssText = "background:rgba(26,111,212,0.08);border:1px solid rgba(26,111,212,0.25);border-radius:6px;padding:6px 12px;font-size:12px;color:#1a4a7a;display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap";
      studentReportBanner.innerHTML = '<span style="font-size:14px">📊</span><span><strong>تقرير أداء الطلاب:</strong> شاهد إنجاز الطلاب في النشاط والواجب والاختبار والإثراء لهذه الحصص.</span>';

      var reportBtn = document.createElement("button");
      reportBtn.textContent = "عرض التقرير";
      reportBtn.style.cssText = "background:#1a6fd4;color:#fff;border:0;border-radius:6px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer;font-family:Cairo,Segoe UI,Tahoma,sans-serif";
      reportBtn.onclick = function () {
        console.log('[Moeen-2] Student Report button clicked (feature in development)');
        if (typeof window.showStudentReport === 'function') {
          window.showStudentReport();
        } else {
          alert('ميزة تقرير الطلاب قيد التطوير. سيتم تفعيلها قريباً.');
        }
      };
      studentReportBanner.appendChild(reportBtn);

      inner.appendChild(sevenDayWarning);
      inner.appendChild(studentReportBanner);

      // ── Row 1: branding · counter · status · save ────────────────────────────
      var row1 = document.createElement("div");
      row1.style.cssText = "display:flex;align-items:center;gap:14px;justify-content:space-between;flex-wrap:wrap";
      ...
      logoImg.src = chrome.runtime.getURL('logo/logo-48.png');
      logoImg.style.cssText = 'width:28px;height:28px;border-radius:6px;object-fit:contain';
      ...
      subtitleEl.style.cssText = "font-size:10px;color:#8a9ab0;margin-top:1px;font-weight:500";
      ...
      centerGroup.style.cssText = "display:flex;align-items:center;gap:12px;flex:1;justify-content:center;flex-wrap:wrap";

      var badge = document.createElement("div");
      badge.className = "Moeen-2-dashboard-badge";
      badge.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px;flex-shrink:0"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>';
      var counterSpan = document.createElement("span");
      counterSpan.id = "Moeen-2-dashboard-counter";
      counterSpan.textContent = "0 \u0645\u0646 0"; // 0 من 0
      ...
      statusDot.style.cssText = "width:7px;height:7px;border-radius:50%;background:#8a9ab0;flex-shrink:0;animation:Moeen-2DotPulse 1.6s ease-in-out infinite";
      var statusEl = document.createElement("div");
      statusEl.id = "Moeen-2-dashboard-status";
      statusEl.style.cssText = "font-size:12px;color:#6b7c93;white-space:nowrap;font-weight:500";
      statusEl.textContent = "\u062C\u0627\u0631\u064A \u0641\u062D\u0635 \u0627\u0644\u062C\u062F\u0648\u0644..."; // جاري فحص الجدول...
      statusWrap.append(statusDot, statusEl);
      centerGroup.append(badge, statusWrap);
      ...
      saveBtn.style.cssText = [
        ...
        "cursor:not-allowed",
        "font-family:Cairo,Segoe UI,Tahoma,sans-serif",
        "transition:transform .2s cubic-bezier(.16,1,.3,1),opacity .2s,box-shadow .2s",
        "opacity:0.45",
        "white-space:nowrap",
        "display:flex",
        "align-items:center",
      ...
      saveBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg><span>\u062D\u0641\u0638 \u0648\u0628\u062F\u0621 \u0627\u0644\u062A\u062D\u0636\u064A\u0631</span>';
      saveBtn.addEventListener("mousedown", function () { if (!saveBtn.disabled) saveBtn.style.transform = "scale(0.96)"; });
      ...
      saveBtn.addEventListener("click", async function () {
        ...
        saveBtn.disabled = true;
        saveBtn.classList.remove("Moeen-2-save-pulse");
        var _savSpan = saveBtn.querySelector("span");
        if (_savSpan) _savSpan.textContent = "\u062C\u0627\u0631\u064A \u062A\u062D\u0636\u064A\u0631 \u0627\u0644\u062D\u0635\u0635\u2026";
        try { await handleDashboardSave(); } finally {
          if (_savSpan) _savSpan.textContent = "\u062D\u0641\u0638 \u0648\u0628\u062F\u0621 \u0627\u0644\u062A\u062D\u0636\u064A\u0631";
          updateDashboardCounter();
        }
      });

      row1.append(branding, centerGroup, saveBtn);

      // ── Row 2: resource pill toggles ─────────────────────────────────────────
      ...
      resLabel.style.cssText = "font-size:10px;color:#8a9ab0;font-weight:700;white-space:nowrap;letter-spacing:0.6px;flex-shrink:0";
      resLabel.textContent = "\u0627\u0644\u0645\u0648\u0627\u0631\u062F \u0627\u0644\u0645\u064F\u0636\u0627\u0641\u0629:"; // الموارد المُضافة:

      var resSep = document.createElement("div");
      resSep.style.cssText = "width:1px;height:18px;background:rgba(0,0,0,0.1);flex-shrink:0";

      var chipsRow = document.createElement("div");
      chipsRow.style.cssText = "display:flex;gap:6px;flex-wrap:wrap";

      var _dbResDefs = [
        { key: 'activity', label: '\u0646\u0634\u0627\u0637', color: '#1a6fd4', bgOn: 'rgba(26,111,212,0.1)', borderOn: 'rgba(26,111,212,0.5)' },
        { key: 'homework', label: '\u0648\u0627\u062C\u0628', color: '#1a9448', bgOn: 'rgba(26,148,72,0.1)', borderOn: 'rgba(26,148,72,0.5)' },
        { key: 'exam', label: '\u0627\u062E\u062A\u0628\u0627\u0631', color: '#c87f0a', bgOn: 'rgba(200,127,10,0.1)', borderOn: 'rgba(200,127,10,0.5)' },
        { key: 'enrichment', label: '\u0625\u062B\u0631\u0627\u0621', color: '#7c3aed', bgOn: 'rgba(124,58,237,0.1)', borderOn: 'rgba(124,58,237,0.5)' }
      ];

      function _applyPillStyle(pill, cb, rd) {
        if (cb.checked) {
          pill.style.background = rd.bgOn;
          pill.style.borderColor = rd.borderOn;
          pill.style.color = rd.color;
          pill.style.boxShadow = '0 2px 8px ' + rd.color + '30';
          pill.style.opacity = '1';
        } else {
          pill.style.background = 'rgba(0,0,0,0.03)';
          pill.style.borderColor = 'rgba(0,0,0,0.1)';
          pill.style.color = '#b0bac7';
          pill.style.boxShadow = 'none';
          pill.style.opacity = '0.75';
        }
      }

      for (var _drd of _dbResDefs) {
        (function (rd) {
          // Use a plain div + explicit click handler — avoids hidden-checkbox label toggle bugs.
          var pill = document.createElement("div");
          pill.className = "Moeen-2-res-pill";
          pill.style.cursor = "pointer";

          // Synthetic state object — _applyPillStyle reads .checked from this.
          var _ps = { checked: getResourceEnabled(rd.key) };

          var dot = document.createElement("span");
          dot.style.cssText = "width:6px;height:6px;border-radius:50%;background:" + rd.color + ";flex-shrink:0;transition:transform .2s";
          dot.style.transform = _ps.checked ? "scale(1)" : "scale(0.7)";

          var txt = document.createElement("span");
          txt.textContent = rd.label;

          var checkMark = document.createElement("span");
          checkMark.className = "Moeen-2-res-pill-check";
          checkMark.textContent = _ps.checked ? "\u2713" : "";
          checkMark.style.opacity = _ps.checked ? "1" : "0";

          _applyPillStyle(pill, _ps, rd);

          pill.addEventListener("click", function () {
            _ps.checked = !_ps.checked;
            setResourceEnabled(rd.key, _ps.checked);
            checkMark.textContent = _ps.checked ? "\u2713" : "";
            checkMark.style.opacity = _ps.checked ? "1" : "0";
            dot.style.transform = _ps.checked ? "scale(1)" : "scale(0.7)";
            _applyPillStyle(pill, _ps, rd);
            updateDashboardCounter(); // re-evaluate save button enabled state
          });

          pill.append(dot, txt, checkMark);
          chipsRow.appendChild(pill);
        })(_drd);
      }

      var resHint = document.createElement("span");
      resHint.id = "Moeen-2-res-hint";
      resHint.textContent = "يجب اختيار مورد واحد على الأقل لبدء التحضير";
      resHint.style.cssText = [
        "font-size:11px",
        "color:#c87f0a",
        "font-family:Cairo,Segoe UI,Tahoma,sans-serif",
        "margin-right:auto",
        ...
        "border:1px solid rgba(200,127,10,0.25)",
        "border-radius:10px",
        "display:none",
        "white-space:nowrap",
        "flex-shrink:0"
      ...
      row2.append(resLabel, resSep, chipsRow, resHint);

      inner.append(row1, row2);
      panel.append(topAccent, inner);
    ...
    async function injectDashboardUI() {
      ...
      updateDashboardStatus("جاري تحميل بيانات جدولك الدراسي...", "loading");

      // Performance optimization: Prefetch lesson data for visible subjects early
      // so dropdowns appear faster when the user starts selecting.
      setTimeout(function () {
        try {
          var cards = document.querySelectorAll('td.day-cell div[data-data], div.cs-lesson-card[data-data]');
          var seen = new Set();
          cards.forEach(function (card) {
            ...
            if (!sid) {
              ...
              if (anchor && anchor.href) {
                var m = anchor.href.match(/subjectId=(\d+)/i);
                if (m) sid = m[1];
              }
            }
            if (sid && !seen.has(sid)) {
              seen.add(sid);
              getLocalSubjectData(sid, null); // warms the cache
            }
          });
        } catch (e) {}
      }, 300);
      ...
      async function processDashboardQueue() {
        ...
        // Batch mode: drain current queue, fetch unique subjects in parallel, then render all at once
        var currentBatch = [];
        while (dashboardQueue.length > 0) {
          currentBatch.push(dashboardQueue.shift());
        }
        ...
        updateDashboardStatus('جاري تحميل الدروس المتاحة...', 'loading');

        // Collect unique subjectIds + extract per-item data
        var uniqueSubjects = new Map();
        for (var item of currentBatch) {
          var d = item.div;
          var sId = d.getAttribute('data-subject-id');
          if (!sId) {
            var c = d.closest('td') || d.parentElement;
            var a = c ? c.querySelector('a') : null;
            if (a && a.href) {
              var m = a.href.match(/subjectId=(\d+)/i);
              if (m) sId = m[1];
            }
          }
          var sName = null;
          var hh = d.querySelector('h2');
          if (hh) sName = hh.innerText.trim();

          item._sid = sId;
          item._sname = sName;

          if (sId && !uniqueSubjects.has(sId)) {
            uniqueSubjects.set(sId, { subjectId: sId, subjectName: sName });
          }
        }

        // Fetch all unique subjects in parallel (cache hits are instant)
        var fetchP = [];
        for (var u of uniqueSubjects.values()) {
          fetchP.push(fetchLessonTreeOptions(u.subjectId, u.subjectName));
        }
        var fetched = await Promise.all(fetchP);

        var optsMap = new Map();
        var k = 0;
        for (var u of uniqueSubjects.values()) {
          optsMap.set(u.subjectId, fetched[k++]);
        }

        // Render all selects using pre-fetched options
        for (var item of currentBatch) {
          var div = item.div;
          var tok = item.token;
          var options = optsMap.get(item._sid) || [];

          if (!document.contains(div)) continue;
          ...
          if (options.length > 0) {
            ...
            var defaultOpt = document.createElement('option');
            defaultOpt.value = '';
            defaultOpt.textContent = '\u0627\u062E\u062A\u0631 \u0627\u0644\u062F\u0631\u0633...';
            select.appendChild(defaultOpt);
            ...
            select.addEventListener('change', function () {
              ...
              if (myToken) {
                var siblings = document.querySelectorAll('select.Moeen-2-dashboard-select[data-lesson-token="' + myToken + '"]');
                for (var sib of siblings) {
                  if (sib === changedSelect) continue;
                  sib.value = newValue;
                  if (newValue) {
            ...
            if (!div.querySelector('.Moeen-2-dashboard-select')) {
        ...
        // Final status
        var totalSelects = document.querySelectorAll('.Moeen-2-dashboard-select').length;
        if (totalSelects > 0) {
          updateDashboardStatus(
            'اختر درساً لكل حصة ثم انقر «حفظ وبدء التحضير» — ' + totalSelects + ' حصة متاحة',
            'info'
          );
        } else if (dashboardLessonCount > 0) {
          updateDashboardStatus(
            'تعذّر تحميل الدروس — تأكد من الاتصال بالإنترنت وتسجيل الدخول وأعد المحاولة',
            'warning'
          );
        }
      ...
      setInterval(function () {
        var lessons = document.querySelectorAll(
          'td.day-cell div[data-data]:not(.Moeen-2-processed),' +
          'div.cs-lesson-card[data-data]:not(.Moeen-2-processed)'
        );
        if (lessons.length === 0) return;

        for (var lessonDiv of lessons) {
          lessonDiv.classList.add('Moeen-2-processed');
